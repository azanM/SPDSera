...
namespace eSPD.Core
{
    public static class EmailCore
    {
        ...
        public static string PrepareMailBodyWithADHApproval(string templateName, object param, object param2)
        {
            string body = GetMailBodyOfTemplate(templateName);

            List<object> ListObject = new List<object>();
            ListObject.Add(param);
            ListObject.Add(param2);
          ...
          ESPDInformation espdInfo)
        {
            using (dsSPDDataContext ctx = new dsSPDDataContext())
            {
                ...
                if (spd.status.Contains("PIC Budget") && tamplate.Contains("SPDADH"))
                {
                    espdEmail.statusName = espdEmail.statusName + "-" + approval.namaLengkap;
                }
                ...
                {
                    trClaim dc = ctx.trClaims.FirstOrDefault(o => o.noSPD == spd.noSPD);

                    if (dc.isCompletedGA == true && statusClaim == "Menunggu approval GA")
                    {
                        tamplate = "ApprovalCompleteClaim.txt";
                    }
                    if (dc.isCompletedGA == true && statusClaim == "Menunggu approval Finance")
                    {
                        tamplate = "InformasiCompleteClaim.txt";
                    }
                    #region claim
                    ...
                    int TotalTunjanganKejauhan = dc.UangTunjanganKejauhan != null ? dc.UangTunjanganKejauhan.Value : 0 +
                                                 dc.UangTunjanganKejauhan2 != null ? dc.UangTunjanganKejauhan2.Value : 0 +
                                                 dc.UangTunjanganKejauhan3 != null ? dc.UangTunjanganKejauhan3.Value : 0 +
                                                 dc.UangTunjanganKejauhan4 != null ? dc.UangTunjanganKejauhan4.Value : 0 +
                                                 dc.UangTunjanganKejauhan5 != null ? dc.UangTunjanganKejauhan5.Value : 0;


                    espdEmail.CTunjanganKejauhan = TotalTunjanganKejauhan == 0 ? TotalTunjanganKejauhan.ToString("#,###") : "";
                ...
                espdEmail.EditUrl = "http://trac39/SPD/";
                espdEmail.EditUrlExternal = "http://118.97.80.12/SPD/";

                //send to requestor and approval
                string body = string.Empty;
                if (indexLevel == "0" && espdInfo != null)
                {
                    body = EmailCore.PrepareMailBodyWithADHApproval(tamplate, espdEmail, espdInfo);
                }
                else
                {
                    body = EmailCore.PrepareMailBodyWith(tamplate, espdEmail);
                }
                ...
                //yosafat.siregar@sera.astra.co.id
        ...
        public static void ApprovalSPD(string nrpApproval, string index, trSPD dataSPD, ESPDInformation espdInfo)
        {
            string TemplateEmail = String.Empty;

            if (index == "0")
            {
                if(dataSPD.isRevisi == true)
                {
                    TemplateEmail = "ApprovalRevisiSPDADH.txt";
                }
                else
                    TemplateEmail = "ApprovalSPDADH.txt";
            }
            else
            {
                if (dataSPD.isRevisi == true)
                {
                    TemplateEmail = "ApprovalRevisiSPD.txt";
                }
                else
                {
                    TemplateEmail = "ApprovalSPD.txt";
                }
            }
        ...
        public static void InformasiSPD(string nrpApproval,trSPD dataSpd)
        {
            string emailTemplate = "InformasiSPD.txt";
            if (dataSpd.isRevisi == true)
                emailTemplate = "InformasiRevisiSPD.txt";
            EmailCore.sendEmailNew(
               nrpApproval,
               "Informasi SPD " + dataSpd.noSPD,
               emailTemplate,
               dataSpd,
               string.Empty,
               string.Empty,
