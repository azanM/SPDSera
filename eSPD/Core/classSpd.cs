    ...
    public class classSpd
    {
       ...
       // emailMessage.Append("Jika dalam waktu 5 hari kerja dari email ini dikirim dokumen bukti fisik masih belum diterima oleh GA maka item claim tersebut di atas dinyatakan hangus.");
        ...
        private StringBuilder ContentClaimMail(StringBuilder emailMessage, trClaim claim, msKaryawan kar)
        {
            ...
            {
                emailMessage.Append("- " + "Lampiran / bukti boarding pass");
                emailMessage.Append("<br />");
            }

            if (claim.hotelTanpaPenginapan_cek == true)
            {
                //emailMessage.Append("- " + "Lampiran/Tanpa Penginapan(Mohon dibuatkan berita acara dengan approval ADH cabang tujuan)");
                emailMessage.Append("- " + "Lampiran/Tanpa Penginapan(Mohon dibuatkan form pengajuan claim penggantian fasilitas tanpa penginapan yang ditandatangani oleh Dept Head HO/BM kota tujuan dan disetujui oleh atasan)");
                emailMessage.Append("<br />");
            }
