    ...
    public class ApprovalSPDUrl : IHttpHandler
    {
        ...
        private void CancelSPD(trSPD dataSpd, string nrp, string nospd)
        {
            using (var ctx = new dsSPDDataContext())
            {
                ...
                // ess siska - update status attandence menjadi cancel 
                using (var ctp = new ESSDataContext())
                {
                    string tglBeragkat = dataSpd.tglBerangkat.ToShortDateString() + " " + dataSpd.jamBerangkat + ":" + dataSpd.menitBerangkat + ":" + "00.000";
                    string tglKembali = dataSpd.tglKembali.ToShortDateString() + " " + dataSpd.jamKembali + ":" + dataSpd.menitKembali + ":" + "00.000";

                    int NrpKaryawan = int.Parse(nrp);
                    var ListDataESS = ctp.TrxAttendances.Where(x => x.Nrp == NrpKaryawan && x.BegDa >= Convert.ToDateTime(tglBeragkat) && x.EndDa < (Convert.ToDateTime(tglKembali)).AddDays(1)).ToList();

                    if (ListDataESS != null)
                    {
                        foreach (var item in ListDataESS)
                        {
                            item.Status = "WFSTS_04";
                            ctp.SubmitChanges();
                        }
                    }
                    else
                    {
                        ListDataESS = ctp.TrxAttendances.Where(x => x.Nrp == NrpKaryawan && x.Reason.Contains(dataSpd.noSPD)).ToList();
                        foreach (var item in ListDataESS)
                        {
                            item.Status = "WFSTS_03";
                            ctp.SubmitChanges();
                        }
                    }
                    //var dataess = ctp.TrxAttendances.FirstOrDefault(o => o.BegDa == Convert.ToDateTime(tglBeragkat) && o.EndDa == Convert.ToDateTime(tglKembali));
                    //dataess.Status = "WFSTS_04";
                    //ctp.SubmitChanges();                   
                }
        ...
        private string RejectSPD(trSPD dataSpd, string nrp, string nospd, string index)
        {
            using (var ctx = new dsSPDDataContext())
            {
                ...
                using (var ctp = new ESSDataContext())
                {
                    ...
                    int NrpKaryawan = int.Parse(dataSpd.nrp);
                    var ListDataESS = ctp.TrxAttendances.Where(x => x.Nrp == NrpKaryawan && x.BegDa >= Convert.ToDateTime(tglBeragkat) && x.EndDa < (Convert.ToDateTime(tglKembali)).AddDays(1)).ToList();
                ...
                if (string.IsNullOrEmpty(returner))
                {
                    ...
                    EmailCore.InformasiSPD(dataSpd.dibuatOleh,dataSpd);
                }
            ...
            try
            {
                using (var ctx = new dsSPDDataContext())
                {
                    ...
                    if (index == "0")
                    {
                        ...
                        dtNoSPD.Rows.Add(dataSpd.noSPD);

                        var SPDInformation = ESPDInformation.CheckBudgetToSAP(dataSpd);//ambil informasi budget dari SAP
                        ...
                        if (SPDInformation.Message == "Success")
                        {
                            ...
                            if (SisaUangSaku < 0)
                                message.Append("Budget Uang Saku Tidak Mencukupi(" + SisaUangSaku.ToString("N2", new CultureInfo("id-ID")) + ")");
                            if (SisaUangMakan < 0)
                                message.Append("Budget Uang Makan Tidak Mencukupi(" + SisaUangMakan.ToString("N2", new CultureInfo("id-ID")) + ")");
                            if (SisaTransportasi < 0)
                                message.Append("Budget Uang Transportasi Tidak Mencukupi(" + SisaTransportasi.ToString("N2", new CultureInfo("id-ID")) + ")");
                            if (SisaAkomodasi < 0)
                                message.Append("Budget Uang Akomodasi Tidak Mencukupi(" + SisaAkomodasi.ToString("N2", new CultureInfo("id-ID")) + ")");
                    ...
                    var approval = ctx.ApprovalStatus.Where(o => o.NoSPD == nospd).OrderBy(o => o.IndexLevel).ToList();// get all approval                    
                    int? indexInt = null;// get current approval
                    if (!string.IsNullOrEmpty(index)) indexInt = Convert.ToInt32(index);
                    var currentApproval = approval.FirstOrDefault(o => o.NrpApproval == nrp && o.Status == null && o.IndexLevel == indexInt);

                    if (currentApproval != null)
                    {
                        if (currentApproval.IndexLevel > 0)
                        {
                            var prevApproval = approval.Where(x => x.IndexLevel == currentApproval.IndexLevel - 1).FirstOrDefault();
                            if (prevApproval.Status == null)
                            {
                                return "SPD Gagal Approve, SPD masih " + prevApproval.ApprovalRule.Deskripsi;
                            }
                        }
                    }

                    var countCekApproval = approval.Where(o => o.Status == null || o.Status == false).Count();// approval tujuan, dan pastikan approval semua sudah diapprove
                    if (nrp == dataSpd.nrpApprovalTujuan && currentApproval == null && string.IsNullOrEmpty(index) && countCekApproval == 0)
                    {
                        ...
                        else if (DateTime.Now.Date < dataSpd.tglBerangkat.Date)//cuman boleh ngeprove hari berangkat ato udah nyampe,
                        {
                        ...
                        else
                        {
                            ...
                            dataSpd.diubahTanggal = DateTime.Now;
                            dataSpd.isApprovedDate = DateTime.Now;
                            ctx.SubmitChanges(); //tutup kalo debug production                            
                            EmailCore.InformasiSPD(dataSpd.dibuatOleh,dataSpd);// send informasi ke pembuat //tutup kalo debug production
                            return string.Empty;
                        }
                    }
                    ...
                    {                        
                        var nextApproval = approval.FirstOrDefault(o => o.IndexLevel == (currentApproval.IndexLevel + 1));// get next approval                       
                        if (nextApproval != null)//set status menunggu approval selanjutnya
                        {
                            ...
                            EmailCore.ApprovalSPD(nextApproval.NrpApproval, indexNext, dataSpd, null); //TO DO Email // send email approval Atasan atau tujuan //tutup kalo debug production
                            EmailCore.InformasiSPD(dataSpd.dibuatOleh,dataSpd);

                            //TO DO Email // send informasi ke pembuat //tutup kalo debug production

                        }
                        else //jika tidak ada next approval lagi
                        {
                            var data = (from a in ctx.trSPDs //cek uang muka, jika lebih dari 0 maka akan generate text
                                        where a.noSPD == nospd
                                        select new
                                        {
                                            uangMuka = a.uangMuka
                            ...
                            if (_uangMuka > 0)
                            {
                                ...
                                string respMessage = GenerateText(nospd, ctx); //generate text //tutup kalo debug production
                                if (respMessage == "success")
                                {
                                    using (var ctp = new ESSDataContext())
                                    {
                                        ...
                                        // ess siska
                                        string errorMessage = string.Empty;
                                        List<TrxAttendance> listObject = new List<TrxAttendance>();
                                        listObject = InsertJadwaltoESS(dataSpd, nrp, ref errorMessage);
                                        if (string.IsNullOrEmpty(errorMessage))
                                        {
                                            ctp.TrxAttendances.InsertAllOnSubmit(listObject); //tutup kalo debug production
                                            ctp.SubmitChanges(); //tutup kalo debug production
                                        }
                                        else
                                        {
                                            return errorMessage;
                                    ...
                                    //AllDatesInAMonth(Convert.ToDateTime(tglBerangkat), Convert.ToDateTime(tglKembali), nrp, dataSpd.dibuatOleh);
                                }
                                else
                                    return respMessage;
                            ...
                            else
                            {
                                ...
                                using (var ctp = new ESSDataContext())
                                {
                                    ...
                                    if (string.IsNullOrEmpty(errorMessage))
                                    {
                                        ...
                                        {
                                            return e.Message + " " + "Error saat insert jadwal ESS" + " " + ctp.Connection.ConnectionString;
                                        }
                                    }
                                    else
                                    {
                            ...
                            EmailCore.ApprovalSPD(dataSpd.nrpApprovalTujuan, string.Empty, dataSpd, null);// send email approval Atasan atau tujuan //TO DO Email  //tutup kalo debug production                            
                            EmailCore.InformasiSPD(dataSpd.dibuatOleh,dataSpd); // send informasi ke pembuat //TO DO Email //tutup kalo debug production
                            EmailCore.InformasiSPDExpired(dataSpd.dibuatOleh, dataSpd.noSPD, dataSpd); //TO DO Email //tutup kalo debug production                            
                            if (dataSpd.tiket.ToLower().Contains("dicarikan"))//send email ke GA
                            {
                                // ===== TO DO Email ===== start
                                ...
                                if (dsCheckBcc.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    for (int i = 0; i < dvCheckBcc.Count; i++)
                                    {
                                        ...
                                        EmailCore.InformasiPencarianTiket(nrpGA, dataSpd.noSPD, dataSpd); //tutup kalo debug production
                                    }
                                }
                                // ===== TO DO Email ===== end
                            }
                    ...
                    {// jika pernah approve                        
                        var hasApproved = approval.Where(o => o.NrpApproval == nrp && o.Status != null);// find has approved
                        ...
                        {                            
                            return string.Empty; // probabilitas 1% karena pasti selalu masuk if induk terlebih dahulu, kecuali anomali data
                        }
                    }
                }
            }
        ...
        //public List<DateTime> AllDatesInAMonth(DateTime tglBerangkat, DateTime tglKembali, String nrp, String dibuatOleh)
        //{
        //    using (var ctp = new ESSDataContext())
        //    {


        //        var firstOftargetMonth = tglBerangkat;
        //        var firstOfNextMonth = tglKembali;

        //        var allDates = new List<DateTime>();

        //        for (DateTime date = firstOftargetMonth; date < firstOfNextMonth; date = date.AddDays(1))
        //        {
        //            TrxAttendance ess = new TrxAttendance();
        //            Guid id = Guid.NewGuid();
        //            ess.Id = id;
        //            ess.Nrp = Convert.ToInt32(dataSpd.dibuatOleh);
        //            ess.AttendanceCode = "8000";
        //            ess.BegDa = date;
        //            ess.EndDa = Convert.ToDateTime(tglKembali);
        //            ess.CreDa = DateTime.Now;
        //            ess.Status = "WFSTS_02";
        //            ess.AppRejDa = DateTime.Now;
        //            ess.AppRejBy = nrp;
        //            ess.DestApp = nrp;
        //            ctp.TrxAttendances.InsertOnSubmit(ess);
        //            ctp.SubmitChanges();
        ...
        public string GenerateText(string nospd, dsSPDDataContext dc)
        {
            try
            {
                ...
                message += data.tglBerangkat.Day + "." + data.tglBerangkat.Month + "." + data.tglBerangkat.Year + "|";//penambahan
                message += data.tglKembali.Day + "." + data.tglKembali.Month + "." + data.tglKembali.Year + "|";//penambahan
                message += _TunjanganKejauhan.Trim();//penambahan
        ...
        public List<TrxAttendance> InsertJadwaltoESS(trSPD dataSpd, string nrp, ref string message)
        {
            List<TrxAttendance> listObject = new List<TrxAttendance>();
            string constr = ConfigurationManager.ConnectionStrings["SPDConnectionString"].ConnectionString;
            try
            {                
                string tglBerangkat = dataSpd.tglBerangkat.ToShortDateString() + " " + dataSpd.jamBerangkat + ":" + dataSpd.menitBerangkat + ":" + "00.000";
                string tglKembali = dataSpd.tglKembali.ToShortDateString() + " " + dataSpd.jamKembali + ":" + dataSpd.menitKembali + ":" + "00.000";

                int Counter = Convert.ToInt32(((Convert.ToDateTime(tglKembali).Date) - (Convert.ToDateTime(tglBerangkat)).Date).TotalDays) + 1;
                DataTable dtListJadwalSPD = new DataTable();
                DataTable dt = new DataTable();
                DataColumn dcListJadwalSPD = new DataColumn("DateList", typeof(string));
                dtListJadwalSPD.Columns.Add(dcListJadwalSPD);
                List<string> ListTglSPD = new List<string>();

                for (int i = 0; i < Counter; i++)
                {
                    DataRow dtRow = dtListJadwalSPD.NewRow();
                    dtRow[0] = dataSpd.tglBerangkat.AddDays(i).ToShortDateString();
                    dtListJadwalSPD.Rows.Add(dtRow);
                    ListTglSPD.Add(dataSpd.tglBerangkat.AddDays(i).ToShortDateString());
                }
                using (SqlConnection con = new SqlConnection(constr))
                {
                    using (SqlCommand cmd = new SqlCommand("SP_GetJadwalKerjaByNRP"))
                    {
                        cmd.Parameters.AddWithValue("@SPDJadwal", dtListJadwalSPD);
                        cmd.Parameters.AddWithValue("@NRP", dataSpd.nrp);
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    ess.AppRejBy = nrp;
                    ess.DestApp = nrp;
                    ess.Reason = dataSpd.noSPD;
                    if (i == 0)
                    {
                        ess.BegDa = Convert.ToDateTime(tglBerangkat);
                        string JamPulang = dataSpd.tglBerangkat.ToShortDateString() + " 23:00:" + "00.000";
                        ess.EndDa = Convert.ToDateTime(JamPulang);
                    }
                    else
                    {
                        if (int.Parse(dt.Rows[i]["PlanHours"].ToString()) == 0)
                        {
                            ess.BegDa = Convert.ToDateTime(dataSpd.tglBerangkat.AddDays(i).ToShortDateString() + " 00:00:" + "00.000");
                            if (i == (Counter - 1))
                                ess.EndDa = Convert.ToDateTime(tglKembali);
                            else
                            {
                                ess.EndDa = Convert.ToDateTime(dataSpd.tglBerangkat.AddDays(i).ToShortDateString() + " 23:00:" + "00.000");
                            }
                        }
                        else
                        {
                            ess.BegDa = Convert.ToDateTime(dataSpd.tglBerangkat.AddDays(i).ToShortDateString() + " " + dt.Rows[i]["StartTime"].ToString());
                            if (i == (Counter - 1))
                                ess.EndDa = Convert.ToDateTime(tglKembali);
                            else
                            {
                                ess.EndDa = Convert.ToDateTime(dataSpd.tglBerangkat.AddDays(i).ToShortDateString() + " " + dt.Rows[i]["EndTime"].ToString());
                            }
                        }
                    }
                    listObject.Add(ess);
            ...
            {
                message = e.Message.ToString() + " " + e.StackTrace.ToString() + " " + "error saat get jadwal dari ESS";
            }
            return listObject;
        }
    }
